...
contract LiquidationOperator is IUniswapV2Callee {
    ...
    function uniswapV2Call(
        ...
        console.log("Borrowed USDT (amount1):", amount1);
        console.log("Pool1 Reserve WBTC:", reserve_WBTC_Pool1);
        console.log("Pool1 Reserve USDT:", reserve_USDT_Pool1);

        // 2.1 liquidate the target user
        uint256 debtToCover = amount1;
        USDT.approve(address(lendingPool), debtToCover);
        (uint256 resWBTC_After, uint256 resUSDT_After, ) = uniswapV2Pair_WBTC_USDT.getReserves();

        console.log("Liquidating target...");
        lendingPool.liquidationCall(address(WBTC), address(USDT), liquidationTarget, debtToCover, false);
        
        uint256 collateral_WBTC = WBTC.balanceOf(address(this));
        console.log("Collateral WBTC received:", collateral_WBTC);

        uint256 repay_WBTC = getAmountIn(debtToCover, reserve_WBTC_Pool1, reserve_USDT_Pool1);
        console.log("WBTC needed to repay Uniswap:", repay_WBTC);

        if (collateral_WBTC < repay_WBTC) {
            console.log("ERROR: Liquidation is NOT profitable!");
            revert("Insolvent Liquidation");
        }

        uint256 leftover_WBTC = collateral_WBTC - repay_WBTC;
        console.log("Profit WBTC (leftover):", leftover_WBTC);

        // 2.3 Swap Profit to WETH
        WBTC.transfer(address(uniswapV2Pair_WBTC_WETH), leftover_WBTC);
        uint256 amountOut_WETH = getAmountOut(leftover_WBTC, reserve_WBTC_Pool2, reserve_WETH_Pool2);
        
        console.log("Expected WETH profit:", amountOut_WETH);
        uniswapV2Pair_WBTC_WETH.swap(0, amountOut_WETH, address(this), "");

        // 2.4 Repay to Pool1
        WBTC.transfer(address(uniswapV2Pair_WBTC_USDT), repay_WBTC);
        
        console.log("--- DEBUG END: Repaid successfully ---");
    }
}
